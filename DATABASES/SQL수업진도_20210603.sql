...
-- null널 이 중요한 이유: null값이 있으면 검색X(그래서 채워넣어야한다)
...
--(중) 서브쿼리? (select쿼리가 중복되어있는...) 이다.
SELECT ROWNUM, E.* FROM (-- 테이블명
SELECT * FROM emp ORDER BY sal DESC --내림차순 
) E WHERE ROWNUM = 1;
-- 위 서브쿼리문장을 해석 할때는 안쪽부터 해석 
-- 위 정렬에서 1등만 구할때 limit는 Mysql(마리아DB)의 명령어. 오라클X
-- Mysql(마리아DB)에 있는 AI(AutoIncrement)자동증가값 명령이 오라클에서 X
...
-- Instr(문자의 위치를 구하는 함수), Substr(매개변수로 입력한 숫자위치만큼 문자열을 추출하는함수)
-- Lpad(LeftPadding왼쪽여백),Rpad(RightPadding오른쪽여백), 레포트프로그램에서 출력조정시 사용
-- Trim(왼쪽,오른쪽 문자열을 잘라내는 함수)
-- 날짜 sysdate,systimestamp 로 오라클전용 함수로서 게시물입력시간, 회원등록 시간
SELECT to_char(systimestamp, 'yyyy-mm-dd hh24:mi:ss:ff') FROM dual;
-- 위 to_char(날짜를 문자열로 변환)형 변환함수라고 한다.
SELECT sysdate + 1 FROM dual;
SELECT sysdate - 1 FROM dual;
--아래는 6개월간 회원정보 수정이 없는 회원에게 공지서비스를 처리하는 용도
SELECT * FROM
TBL_MEMBER
