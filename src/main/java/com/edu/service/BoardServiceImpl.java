 ...
 *
 */
@Service //@에노테이션을 붙이면 스프링 빈으로 등록이 된다.
public class BoardServiceImpl implements IF_BoardService {
	...
	public int countBoard(PageVO pageVO) throws Exception {
		// TODO 페이징 처리시 PageVO의 totalCount변수에 사용 될 값을 리턴값으로 받음.
	...
	public void deleteBoard(int bno) throws Exception {
		...
		// 트랜잭션이 필요한 순간, 작업순서 1.첨부파일 삭제 OK 2.게시물 삭제시 에러가 있으면 삭제가 안됨.
		// 위와 같은 상황을 방지하는 목적의 @Transaction 에노테이션을 사용한다.
		// 댓글 삭제도 *나중에 추가
		// 특이사항: 첨부파일 DB만 삭제해서 해결 + 실제 업로드된 파일 삭제가 필요 *나중에 추가
	...
	public void insertBoard(BoardVO boardVO) throws Exception {
		// TODO 부모 게시물 insertBoard 실행 -> 자식 첨부파일 있으면 첨부파일 insertAttach
		// 게시물 등록 + 반환값으로 bno추가
		int bno = boardDAO.insertBoard(boardVO);
		// 첨부 파일 등록: 1개 이상일때 가정해서 처리
