...
/**
 * 이 클래스는 Admin관리자단을 접근하는 클래스 <- 디스패처 서블렛(게이트웨이) 기능을 한다.
 * 디스페처 서블렛 클래스는 톰켓이 실행될때 제일 먼저 실행되는 클래스 그래서 게이트웨이라고한다.
 * 디스패쳐 서블렛 실행될때, 컨트롤러의 RequestMapping경로를  재등록한다.
 * 변수Object를 만들어서 jsp로 전송 + jsp에서 값을 받아서 Object로 처리
 * @author 전동수
 *
 */
@Controller
public class AdminController {
	...
	public String board_delete(@RequestParam("bno")Integer bno,PageVO pageVO) throws Exception {
		...
		//기존 등록된 첨부파일 폴더에서 삭제할 UUID(고유한식별값생성클래스)이름을 추출한다.(아래)
		List<AttachVO> delFiles = boardService.readAttach(bno);//해당게시물의 모든 첨부파일 delFile에 임시로 담아 놓는다.
	...
	public String board_view(@RequestParam("bno")Integer bno,@ModelAttribute("pageVO")PageVO pageVO, Model model) throws Exception {
		...
		//배열객체 생성구조: String[] 배열형 = new String[배열크기];
		...
		//위 for은 세로데이터(다수레코드)를 가로데이터(1레코드이면서, 배열)에 담아서 1개 레코드boardVO로 만드는게 목적.
		...
		//업로드한 파일이 이미지인지 아닌지 확인하는 용도의 데이터 입니다.아래(목적:이미지일때 미리보기img태그를 사용해서)
		model.addAttribute("checkImgArray", commonUtil.getCheckImageArray());
	...
	//jsp에서 게시판생성관리에 Get/Post 접근할때 URL을 bbs_type로 지정한다.
	//왜 board_type로 하지않고, bbs_type로 하는 이유는 왼쪽메뉴를 고정시키는 로직에서 경로가 board와 겹치지 않도록 하려고.
	...
	//bbs_type_insert.jsp의 입력폼에서 전송된 값을 boardTypeVO 자동으로담겨서 {구현} 단, 자동으로 값이 바인딩 되려면, 폼의 name과 VO 멤버변수명과 동일해야함.
	...
	public String insertBoardType(BoardTypeVO boardTypeVO) throws Exception {//입력처리1
		...
		//쇼핑몰에서 결제화면을 처리 후 뒤로가기를 누르면, 리아디렉트는 데이터가 사라지기때문에 재결제 불가
		//forward로 결제화면을 처리 후 뒤로가기를 누르면, 재결제가 발생된다. 이러면 안되기때문에 사용안함.
	}
	//게시판 생성관리 이 기능은 사용자단에서 UI를 사용할 일이 없기 때문에 Read, Update를 1개(공통)로 사용.
	...
	public String updateBoardTypeForm(@RequestParam("board_type")String board_type, Model model) throws Exception {//수정폼1
		model.addAttribute("boardTypeVO", boardTypeService.readBoardType(board_type));//서식model.add첫번째는 jsp변수로 담아서 view화면으로보냄, 두번째는 서비스에서 쿼리실행한 데이터객체
	...
	//아래 경로는 회원신규등록 폼을 호출하는 URL쿼리스트링으로 보낸것을 받을때는 GET방식으로 받는다.
	...
	public String updateMember(MemberVO memberVO, PageVO pageVO) throws Exception {
		//update 서비스만 처리하면 끝
		//업데이트 쿼리서비스 호출하기 전 스프링시큐리티 암호화 적용한다.
		String rawPassword = memberVO.getUser_pw();
		if(!rawPassword.isEmpty()) {//수정폼에서 암호를 입력값이 비어있지 않을때만 아래로직실행.
			BCryptPasswordEncoder passwordEncoder = new BCryptPasswordEncoder();
			String encPassword = passwordEncoder.encode(rawPassword);
			memberVO.setUser_pw(encPassword);
		}		
		memberService.updateMember(memberVO);//반환값이 없다.
		//redirect로 페이지를 이동하면, model로 담아서 보낼수 없다. 쿼리스트링(URL?)으로 보낸다.
	...
	public String deleteMember(MemberVO memberVO) throws Exception {
		...
		//MemberVO memberVO는 클래스형 변수, String user_id 스트링형 변수 (둘이 내내 같은방식이다).
		...
		//위 방식대로 하면, 새로고침하면 /admin/member/member_delete가 계속 실행됨.-사용자단에서 실습예정
		//게시판테러상황을 방지하기 위해서, 쿼리를 작업 후 이동할때는 redirect(다시접속)라는 명령을 사용한다.
	...
	public String viewMemberForm(Model model, @RequestParam("user_id")String user_id,@ModelAttribute("pageVO")PageVO pageVO) throws Exception{
		 ...
		 * 다른점은 JUnit에서는 식별자 ID를 강제로 지정했지만, 이 메서드에서는 @RequestParam인터페이스를 이용해서 식별자값을 받음.
		 */		
		//위 출력값 memberVO 1개의 레코드를 model를 이용해서 member_view.jsp 보낸다.(아래)
		model.addAttribute("memberVO",memberService.readMember(user_id));
		//model.addAttributes("pageVO", pageVO);
	//아래 페이지 반환시(렌더링) @ModelAttribute("pageVO")에 의해서 pageVO.page변수값으로 jsp로 보낸다.
	...
	public String admin(Model model) throws Exception {//에러발생시 Exception클래스의 정보를 스프링으로 보내게 된다.
		...
		//아래 상대경로에서 home.jsp에서 .jsp (생략suffix접미어) 이다.
