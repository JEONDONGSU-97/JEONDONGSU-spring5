 ...
 * @author 전동수
 * 컨트롤러 기능 @Controller(jsp와 바인딩이 필요할때는 필수 애노테이션이다.)
 * 컴포넌트 @Component는 MVC가 아닌 기능들을 모아놓은 스프링빈 명시, 여기서는 jsp와 바인딩이 필요해서 사용안함.
 */
@Controller
public class CommonUtil {
	...
	//첨부파일 업로드/다운로드/삭제/인서트/수정에 모두 사용될 저장경로를 1개지정해서 [전역]으로 사용
	...
	public ArrayList<String> getCheckImageArray() {
	...
	public String id_check(@RequestParam("user_id")String user_id) throws Exception {
		...
		return memberCnt;//0.jsp 이렇게 작동하지 않는다. 이유는 @ResponsBody때문에 RestAPI는 값만 반환
	}
}
