...
// 1번 서브메뉴 숨기는 함수(아래)
function goHide1() {
	if(!isOver1 && !isOverSub1) {//2개의 변수값이 false일때 구현조건이 된다.
...
$(document).ready(function(){
	...
	$('.openAll1').mouseout(function(){
		isOver1 = false;
		setTimeout('goHide1()',200);// 기본은 goHide() 이것만하면되는데, mouseout이벤트 놓치면 안되기때문에 setTimeout 사용
	});
	// 서브메뉴1 액션
	$('.gnb_depth2_1').mouseover(function(){
		isOverSub1 = true;//서브메뉴 1번이 계속 나타난다.
	});
	...
	$('.openAll2').mouseout(function(){
		isOver2 = false;
		setTimeout('goHide2()',200);// 기본은 goHide() 이것만하면되는데, mouseout이벤트 놓치면 안되기때문에 setTimeout 사용
	});

	// 서브메뉴2 액션
	$('.gnb_depth2_2').mouseover(function(){
		isOverSub2 = true;//서브메뉴 2번이 계속 나타난다.
	});
