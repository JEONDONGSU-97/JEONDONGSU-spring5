package kr.or.test;
/**
 * 이 클래스는 자바앱에서 개발자가 예외를 처리하는 방법을 실습하는 클래스 이다.
 * @author 전동수
 *
 */
public class ExceptionTest {
	...
	public static void main(String[] args) {
		...
		// 위 처럼(MemberVO) 동일 패키지안의 클래스는  import없이 사용가능 하다.
		memberVO.setName("홍길동");
		memberVO.setAge(10);
		memberVO.setPhoneNum("000-0000-0000");
		System.out.println("한꺼번에 출력하고 싶을때 toString()메서드를 만든다.");
		...
		for(int cnt=0;cnt<=3;cnt++) {
			 ...
			 * finally {(선택사항)에러가 발생하든 아니든 무조건 실행됨} 
			 */
			try {
				indexValue = Integer.parseInt(stringArray[cnt]);
			} catch(NumberFormatException ex) {//Exception 대신에 선별적으로 예외사항을 잡는다.
