...
class MemberVO {
	...
	//위 프라이빗 멤버변수를 입출력을 구현한  메서드를 만든다.
	...
	// String을 반환(return) 겟 메서드 이다.(아래)
	public String getPhoneNum() {
		return phoneNum;
	}
	// void한(반환값이 없이) 입력하는 셋 메서드 이다.(아래)
	public void setPhoneNum(String phoneNum) {
		this.phoneNum = phoneNum;//프라이빗변수를 직접접근을 못하니까 
		//퍼블릭 메서드로 입력하게 만들어 놓은 구조이다. 
	}
...
public class Step2 {
	...
	public static void main(String[] args) {
		...
		//객체로 만들면, 호출(runtime)시 메모리에 로딩된다. -> 실행끝나면, 반환.
		//외부클래스는 아래처럼 직접 접근해서 메서드나 변수를 사용할 수 없음.
		//외부클래스로 접근하려면, 객체(실행가능한=메모리로딩)로 만들어야 가능함.
		//static예약어는 컴파일시 메모리에 로딩(객체가됨)이 되게 명시
