 ...
 * 장점.메모리 낭비를 방지하는 효과
 * 예, DB접속객체를 예를 들면
 * Connection conn1 = dataSource.getConnection();//싱글톤객체 new키워드를 사용X
 * connection객체가 생성되면,
 * Connection conn2 = dataSource.getConnection();//싱글톤객체
 * Connection conn3 = dataSource.getConnection();//싱글톤객체
 * conn1==conn2==conn3 다 같은 동일객체로 같은 메모리를 사용한다. 즉, 1개의 객체만 생성된다.
 * 결과적으로 1번 DB커넥션이 성공하면, 객체가 없어질때까지(null) 중복으로 생성되지 않는다.
