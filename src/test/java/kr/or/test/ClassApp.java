...
class Circle {
	...
	//생성자 메서드(아래)
	//public Circle() { } 만들지 않아도 자동으로 만들어 진다.
...
class Employee { //고용(사원,취업) 부모클래스
	...
	public void doJob() {	
		System.out.println("환엽합니다. 직원 여러분!");
	}
}
class Salesman extends Employee {//extends확장 -> 상속
	public Salesman() { //클래스명과 똑같은 이름의 메서드가 생성자 함수이다.
		// 자동으로 생성되는데, 개발자가 커스터마이징 할때 만든다.(아래)
...
abstract class GraphicObject {
	int x,y;
	abstract void draw();//구현내용이 없고, 메서드 명만 존재. 추상 메서드이다.
	//추상메서드를 만드는 이유는 메서드가 수십개 수백개일때 개발자 구현할때 어려움이 존재
	//위 문제를 해소한 기능의 클래스 수백개의 메서드를 이름만 모아서, 유지보수가 편리하게 처리한 방식이다.
}
// 추상클래스를 사용하는 방법(아래)
// 스프링에서는 추상클래스보다는 인터페이스를 더 많이 사용한다. 사용의도 비슷하다.
class Traiangle extends GraphicObject {
	//삼각형 그리기
	@Override //부모클래스의 메서드를 재정의해서 사용 = 오버라이드 / 오버로드 차이점확인(Step1.java)
	void draw() {
		// 삼각형을 만듬
		System.out.println("  *  ");
		System.out.println(" * * ");
